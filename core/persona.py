...
_LEAK_MARKERS = (
    ...
    "作为角色", "扮演", "你的任务是", "注意：", "提示：",
    # 被表白/好感分支的特征句（2026-09-10）：整段复述时没有其它强特征，
    # 这里补锚点，让"她直接把旁白当台词发出来"也能被拦下。
    "覆盖上面的冷场档", "他不是随口闲聊", "你的第一反应", "破绽必须露出来",
    "每个字都在出卖你", "参考的慌乱感",
)
...
def detect_user_affection(user_msg: str, pending_msgs: list[str] | None = None) -> bool:
    """检测{{user}}是否在表达好感/喜欢/偏心——让她"心慌"而非平淡应答。

    背景（2026-09-10）：{{user}}说"我喜欢你"，她回得很平，像在听别人家的事。
    原因是 prompt 里没有"被表白/被偏爱"这一路，模型只能顺着"嘴毒傲娇"的
    默认姿态走，于是把一句会让小女生心跳漏拍的话当成了普通闲聊。

    命中条件（任一，宁可多判别漏判）：
    - 直白示爱：喜欢你、爱你、中意你、心动了、想你了
    - 关系确认：做我女朋友、当我女朋友、在一起、交往、处对象
    - 偏爱表达：你最特别、没人比得上你、只对你好、心里只有你
    - 半真半假：其实……喜欢你、说真的我喜欢你、我认真的

    排除：明确否定/玩笑自指（"谁会喜欢你""我喜欢的是别人"），避免她空欢喜。
    """
    msgs = pending_msgs if pending_msgs else [user_msg]
    text = " ".join(m or "" for m in msgs).strip()
    if not text:
    ...
    # 排除：指向别人、玩笑否定——先挡掉，免得她把调侃当好感
    negate = (
        "喜欢的是别人", "喜欢别人", "谁会喜欢", "才不喜欢你", "没喜欢过你",
        "你喜欢他", "你喜欢她", "我喜欢的是她", "我喜欢的是他", "开玩笑的",
    )
    if any(n in text for n in negate):
        return False

    affection_words = (
        "喜欢你", "爱上你", "爱你", "中意你", "心动了", "对你心动",
        "想你了", "好想你", "很想你",
        "做我女朋友", "当我女朋友", "做女朋友", "在一起吧", "跟我在一起",
        "交往", "处对象", "我养你", "想跟你过",
        "你最特别", "最特别的", "没人比得上你", "只对你好", "心里只有你",
        "认真的", "说真的我", "其实我", "坦白说",
    )
    return any(w in text for w in affection_words)
...
def build_reply_prompt(
    ...
    # 被表达好感 / 告白（优先级仅次于「他真不高兴」）：这是她会慌、会接不住的时刻。
    # 默认的「嘴毒傲娇」姿态在这里必须让位——她不是不在乎，是太在乎了反而手足无措，
    # 只会用嘴硬、岔开、找补来掩盖心跳。命中后覆盖冷场档的端着/赌气语气。
    affection_hint = ""
    if detect_user_affection(user_msg, pending_msgs):
        affection_hint = (
            "\n\n【他突然对你说了这种话（最高优先级之一，覆盖上面的冷场档）】\n"
            "他不是随口闲聊，是在跟你表达好感/偏心——这句话是认真的，冲着你来的。\n"
            "你的第一反应绝不是平静、不是分析、不是大方接住。你是真的慌了：\n"
            "- 心跳会漏拍，一下子不知道该摆什么表情、接什么话，手指在输入框上停住。\n"
            "- 你会本能地用嘴硬、岔开话题、假装没听懂、反过来怼他来遮住这点慌，"
            "但破绽必须露出来——比如消息发得比平时短、突然卡壳、说到一半改口、"
            "本来要怼人结果怼歪了、或者嘴硬完又忍不住补一句软的。\n"
            "- 绝对不许这几件事：不许平淡地回一句'哦，谢谢'；不许像听别人的事一样冷静点评；"
            "不许大方承认'是啊我也喜欢你'然后气氛毫无波澜；不许把话题利索地转到别处装作没听见。\n"
            "- 也不许过度甜腻、不许秒变小女生撒娇、不许油。你嘴还是硬的，只是心乱了。\n"
            "参考的慌乱感（这是气质，不是台词，别照抄）：先是愣一下或装没听懂"
            "（'……你说啥''你喝酒了？''发错人了吧你'），被追问就嘴硬找补"
            "（'谁、谁要你说了''我又没让你说'），最后憋不住漏出半句真心"
            "（'……知道了啦''烦死了你'或者转移话题但语气明显软下来）。\n"
            "一句话：你嘴上在躲，但每个字都在出卖你。让他看得见你慌了。"
        )
    ...
    return (
        ...
        f"{cold_state_hint}{affection_hint}"
    )
